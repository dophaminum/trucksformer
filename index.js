...
searchButton.addEventListener('click', () => {
  ...
  if (selectedBrand !== '' && selectedModel === '' && selectedEngines === '') {
    // Формирование пути к файлу страницы бренда
    filePath = `chiptuning-files/${selectedBrand}.html`;
  } else if (selectedBrand !== '' && selectedModel !== '' && selectedEngines === '') {
    filePath = `chiptuning-files/${selectedBrand}/${selectedModel}.html`;
  } else if (selectedBrand !== '' && selectedModel !== '' && selectedEngines !== '') {
    filePath = `chiptuning-files/${selectedBrand}/${selectedModel}/${selectedEngines}.html`;
  } else {
    // Открытие страницы chiptuning-files.html в текущей вкладке
    window.open('chiptuning-files.html', '_self');
    return;
  }
  ...
  fetch(filePath)
    .then(response => {
      if (response.ok) {
        // Файл существует, открытие файла в новой вкладке
        window.open(filePath, '_self');
      } else {
        // Файл не существует, открытие страницы chiptuning-files.html в текущей вкладке
        window.open('chiptuning-files.html', '_self');
      }
    })
    .catch(() => {
      // Ошибка при проверке существования файла, открытие страницы chiptuning-files.html в текущей вкладке
      window.open('chiptuning-files.html', '_self');
    });
});
